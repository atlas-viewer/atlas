	...
	return (
		<>
			<div style={{ height: "200vh" }}>
				<div
					style={{ background: "#eee", borderRadius: 3, padding: 20 }}
					id="fs-viewer"
				>
					<h3>Viewer</h3>
					...
					<button
						onClick={() =>
							setRenderPreset(["default-preset", { canvasBox: true }])
						}
					>
						Default preset
					</button>
					<button onClick={() => setRenderPreset(["static-preset", {}])}>
						Static preset
					</button>
					<input
						type="range"
						...
						<div style={{ width: 300 }}>
							...
							{annotations.map((annotation) => (
								<div key={annotation.id}>
									{annotation.id}{" "}
									<button onClick={() => editAnnotation(annotation.id)}>
										edit
									</button>{" "}
...
export const objectFitCover = () => {
	...
	return (
      ...
      `}</style>
			...
			<button
				onClick={() =>
					setCover(options[(options.indexOf(cover) + 1) % options.length])
				}
			>
				{cover.toString()}
			</button>
					...
					return (
						<div key={key}>
							...
							<input
								...
								onChange={(e) =>
									dispatch({ type: key, value: e.target.valueAsNumber / 100 })
								}
							/>
						</div>
					);
