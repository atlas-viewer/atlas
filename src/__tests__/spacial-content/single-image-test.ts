...
describe('SingleImage', () => {
  ...
  test('it can construct scaled image', () => {
    ...
    const [, points] = image.getAllPointsAt(DnaFactory.projection({ x: 0, y: 0, width: 100, height: 200 }))[0];
  ...
  test('can be transformed when requesting points', () => {
    ...
    const [, points, t] = image.getAllPointsAt(
      DnaFactory.projection({ x: 0, y: 0, width: 100, height: 200 }),
      compose(translate(100, 500), scale(4))
    )[0];
