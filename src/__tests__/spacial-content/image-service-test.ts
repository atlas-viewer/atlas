import { CompositeResource, SingleImage, TiledImage } from '../../spacial-content/index';
import { DnaFactory, filterPoints } from '@atlas-viewer/dna';
import { fromContentResource } from '../../modules/image-service-data-source/from-content-resource';

describe('ImageService', () => {
  describe('fromContentResource', () => {
    test('it can create from content resource without service', () => {
      const service = fromContentResource({
    ...
    test('it can construct from NLS example', () => {
      const service = fromContentResource({
        ...
        width: 2500,
      }) as CompositeResource[];

      expect(service.length).toEqual(1);
      expect(service[0].points.length).toEqual(5);
      expect(service[0].images.length).toEqual(5);
      expect(service[0].images[0].points.length).toEqual(400);
      expect(service[0].images[1].points.length).toEqual(100);
      expect(service[0].images[2].points.length).toEqual(30);
      expect(service[0].images[3].points.length).toEqual(10);
      expect(service[0].images[4].points.length).toEqual(5);

      const [paintable, points] = service[0].getAllPointsAt(
        DnaFactory.projection({ x: 0, y: 0, width: 500, height: 500 })
      )[0];

      expect(points.length).toEqual(400);
      expect(paintable).toStrictEqual(service[0].images[0]);
    ...
    test('simple scaled image', () => {
      const service = fromContentResource({
        id: 'https://example.org/image/full/full/0/native.jpg',
        type: 'Image',
        format: 'image/jpeg',
        width: 8076,
        height: 9476,
      ...
      expect(service.length).toEqual(1);
      expect(service[0].images.length).toEqual(6);
    });
  });
});
