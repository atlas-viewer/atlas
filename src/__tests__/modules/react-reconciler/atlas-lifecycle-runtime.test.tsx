...
describe('Atlas lifecycle runtime behavior', () => {
  ...
  test('worldKey swaps stage a second preset and only commit after staging runtime becomes ready', async () => {
    ...
    await act(async () => {
      root.render(
        <Atlas width={300} height={200} unstable_noReconciler worldKey={1} onCreated={onCreated} onReady={onReady}>
          <React.Fragment />
        </Atlas>
      );
      await flush();
    ...
    expect(createdPresets).toHaveLength(1);
    expect(onCreated).toHaveBeenCalledWith(createdPresets[0].preset, { stage: 'active', worldKey: 1 });

    await act(async () => {
      root.render(
        <Atlas width={300} height={200} unstable_noReconciler worldKey={2} onCreated={onCreated} onReady={onReady}>
          <React.Fragment />
        </Atlas>
      );
      await flush();
    ...
    expect(onCreated).toHaveBeenCalledWith(createdPresets[1].preset, { stage: 'staging', worldKey: 2 });
  ...
  test('onCreated callback churn does not retrigger existing active or staging presets', async () => {
    ...
    await act(async () => {
      root.render(
        <Atlas width={300} height={200} unstable_noReconciler worldKey={1} onCreated={firstOnCreated}>
          <React.Fragment />
        </Atlas>
      );
      await flush();
    ...
    await act(async () => {
      root.render(
        <Atlas width={300} height={200} unstable_noReconciler worldKey={2} onCreated={firstOnCreated}>
          <React.Fragment />
        </Atlas>
      );
      await flush();
    ...
    await act(async () => {
      root.render(
        <Atlas width={300} height={200} unstable_noReconciler worldKey={2} onCreated={secondOnCreated}>
          <div />
        </Atlas>
      );
      await flush();
  ...
  test('hard-construction prop changes during a transaction rebuild staging without recreating the active preset', async () => {
    ...
    await act(async () => {
      root.render(
        <Atlas width={300} height={200} unstable_noReconciler worldKey={1} interactionMode="popmotion">
          <React.Fragment />
        </Atlas>
      );
      await flush();
    ...
    await act(async () => {
      root.render(
        <Atlas width={300} height={200} unstable_noReconciler worldKey={2} interactionMode="popmotion">
          <React.Fragment />
        </Atlas>
      );
      await flush();
    ...
    await act(async () => {
      root.render(
        <Atlas width={300} height={200} unstable_noReconciler worldKey={2} interactionMode="pdf-scroll-zone">
          <React.Fragment />
        </Atlas>
      );
      await flush();
